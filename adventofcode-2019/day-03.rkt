...
(struct move (n x y) #:transparent)

(define (wire->moves wire)
  (define orders (string-split wire ","))
  (for/list ((order orders))
    
    (define n (string->number (substring order 1)))
    (define-values (dx dy)
      (match (string-ref order 0)
        [#\R (values 1 0)]
        [#\U (values 0 1)]
        [#\L (values -1 0)]
        [#\D (values 0 -1)]))
...
(define (find-closest-intersection wires)
  (define grid (make-hash))
  
  (define (mark-path wires)
    (for ((wi (in-range 2)))
      (define wire (list-ref wires wi))
      (define moves (wire->moves wire))
  ...
  (hash-for-each
   ...
   (lambda (pos freq)
     (when (= freq 1)
       (define dist (+ (abs (car pos)) (abs (cdr pos))))
...
(define wire1 "R8,U5,L5,D3")
(define wire2 "U7,R6,D4,L4")
(check-eq? (find-closest-intersection (list wire1 wire2)) 6)

(define wire3 "R75,D30,R83,U83,L12,D49,R71,U7,L72")
(define wire4 "U62,R66,U55,R34,D71,R55,D58,R83")
(check-eq? (find-closest-intersection (list wire3 wire4)) 159)

(set! wire1 "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51")
(set! wire2 "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7")
(check-eq? (find-closest-intersection (list wire1 wire2)) 135)
...
(find-closest-intersection (list wire1 wire2))
